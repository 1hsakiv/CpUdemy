...
struct segmenttree
{
    ...
    int query(int start, int ending, int l, int r, int node)
    {
        ...
        if (lazy[node] != 0)
        {
            ...
            // even in query part we are updating means
            // we are only updating only it is required
            st[node] += lazy[node] * (ending - start + 1);
            if (start != ending)
            {
                // propagate the updated value
                // if it is leaf node value can't be propagated
                // that will we handled in complete overlap case
